...
export default function DeliveryPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return allData.filter(r => r.current_location === locFilter)
  }, [allData, locFilter])

  const todayRows       = filtered.filter(r => r.delivery_status === 'today')
  const overdueRows     = filtered.filter(r => r.delivery_status === 'overdue')
  const upcomingRows    = filtered.filter(r =>
    r.delivery_date && r.delivery_status !== 'today' && r.delivery_status !== 'overdue'
  ).sort((a, b) => (a.delivery_date ?? '').localeCompare(b.delivery_date ?? ''))
  const unscheduledRows = filtered.filter(r => !r.delivery_date)
  ...
  const ActionButtons = ({ row }: { row: StockWithDelivery }) => (
    <div className="flex items-center gap-1">
      <button
        onClick={e => { e.stopPropagation(); setWaStock(row) }}
        title="WhatsApp"
      ...
      <button
        onClick={e => { e.stopPropagation(); setDelStock(row) }}
  ...
  return (
    <>
      <Header
        ...
        subtitle={`${filtered.filter(r => r.delivery_date).length} scheduled · ${unscheduledRows.length} unscheduled`}
        actions={
        ...
        {!loading && todayRows.length > 0 && (
          <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
            ...
            <p className="text-sm text-amber-700">
              ...
              {todayRows.some(r => !r.qc_status || r.qc_status === 'pending') &&
          ...
          <div className="space-y-4">
            ...
            <div>
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                Aaj ki Deliveries ({todayRows.length})
              </h3>
