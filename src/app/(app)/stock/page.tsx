...
import {
  fmtDate, fmtDateTime, ageingClass, customerName,
  getDeliveryStatus, truncate
} from '@/lib/utils'
...
export default function StockPage() {
  ...
  const filtered = useMemo(() => {
    let data = allData
    if (locFilter !== 'ALL') {
      data = data.filter(r => r.current_location === locFilter)
    }
    if (search.trim()) {
  ...
  return (
    <>
      <Header
        title="Match Stock"
        subtitle={`${filtered.length} vehicles${locFilter !== 'ALL' ? ` · ${locFilter}` : ''}`}
        actions={
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg
                          px-3 py-2 ml-auto min-w-[260px] focus-within:ring-2 focus-within:ring-brand-500
