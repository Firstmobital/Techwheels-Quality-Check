...
import type { QCChecklistItem, QCPhoto, StockWithDelivery } from '../../types'
...
const PHOTO_LABELS = ['Front', 'Rear', 'Left Side', 'Right Side', 'Interior', 'Engine Bay'] as const
...
export default function QCPage() {
  ...
  async function load() {
    ...
    const qcRows = (qcRecords ?? []) as QCStatusRow[]
    ...
    if (preselectedChassis) {
      const target = enriched.find((r: StockWithDelivery) => r.chassis_no === preselectedChassis)
      if (target) {
        openFormFor(target, form.inspectorName)
      }
    }
  }
  ...
  }, [isManager, isSuperAdmin, locationName, locFilter])

  useEffect(() => {
    if (authUser) {
      const name = [authUser.employee.first_name, authUser.employee.last_name].filter(Boolean).join(' ')
      setForm((f: QCFormState) => ({ ...f, inspectorName: name }))
    }
  }, [authUser])

  const filtered = useMemo(() => {
    let data = allData
    if (locFilter !== 'ALL') data = data.filter((r: StockWithDelivery) => r.current_location === locFilter)
    if (search.trim()) {
  ...
  function handlePhoto(label: string, file: File) {
    ...
    setForm((f: QCFormState) => ({ ...f, photos: { ...f.photos, [label]: { file, preview } } }))
  }

  const passedCount = Object.values(form.checklist as QCFormState['checklist']).filter((v) => v.passed).length
  const progressPct = Math.round((passedCount / CHECKLIST_ITEMS.length) * 100)

  async function submitQC() {
    if (!selected) return
    if (!form.decision) {
      toastError('Approve ya Reject decision zaroori hai')
      return
    }
    setSaving(true)

    const photoUrls: QCPhoto[] = []
    for (const [label, { file }] of Object.entries(form.photos) as Array<[string, { file: File; preview: string }]>) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-slate-800">QC Checklist</h1>
          <p className="text-sm text-slate-500">{filtered.length} vehicles</p>
        </div>
        <button
          onClick={() => { void load() }}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-3 py-2 ml-auto min-w-[240px] focus-within:ring-2 focus-within:ring-brand-500">
          ...
          <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4">
            <div className="w-full md:max-w-2xl bg-white rounded-t-2xl md:rounded-2xl shadow-2xl border border-slate-200 max-h-[95vh] flex flex-col">
              <div className="flex items-start gap-3 px-5 py-4 border-b border-slate-100 shrink-0">
                ...
                <button onClick={() => setSelected(null)} className="text-slate-400 hover:text-slate-600 p-1 rounded">✕</button>
              </div>
                ...
                <div className="p-5 space-y-6">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setForm((f: QCFormState) => ({ ...f, inspectorName: e.target.value }))}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Checklist</label>
                      <span className="text-xs font-semibold text-slate-600">{passedCount}/{CHECKLIST_ITEMS.length}</span>
                    </div>
                      ...
                      {CHECKLIST_ITEMS.map((item) => {
                        ...
                        return (
                          <div key={item.key} className={`rounded-xl border transition-colors ${passed ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                            ...
                            >
                              <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 border-2 transition-colors ${passed ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300 bg-white'}`}>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Photos</label>
                  ...
                  <div>
                    ...
                    <textarea
                      value={form.remarks}
                      onChange={(e) => setForm((f: QCFormState) => ({ ...f, remarks: e.target.value }))}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        type="button"
                        onClick={() => setForm((f: QCFormState) => ({ ...f, decision: 'approved' }))}
                      ...
                      <button
                        type="button"
                        onClick={() => setForm((f: QCFormState) => ({ ...f, decision: 'rejected' }))}
                        className={`flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm border-2 transition-all ${
