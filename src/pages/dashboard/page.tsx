...
import { fmtDate, fmtTime, getDeliveryStatus } from '@/lib/utils'
import {
...
export default function DashboardPage() {
  const { isManager, isSuperAdmin, locationName } = useAuth()
  ...
  async function load() {
    ...
    // For non-super-admins with a location, filter to their location
    ...
    const enriched = relevantStock.map(s => {
  ...
  return (
    <>
      <Header
        title="Dashboard"
        subtitle={isSuperAdmin || isManager ? 'All locations overview' : `${locationName ?? 'Your location'}`}
        actions={
          <button onClick={load}
      ...
      <div className="space-y-6">
        ...
        {stats && stats.deliveryToday > 0 && (
          <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
            ...
            <Link href="/delivery" className="ml-auto text-xs font-semibold text-amber-600 hover:text-amber-800 whitespace-nowrap">
              Dekho →
            </Link>
          </div>
        )}
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="data-table">
                ...
                <tbody>
                  {todayRows.map(row => (
                    <Link key={row.chassis_no} href={`/stock/${encodeURIComponent(row.chassis_no)}`}
                      legacyBehavior>
